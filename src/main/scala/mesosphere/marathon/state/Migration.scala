...
class Migration @Inject() (
  ...
  def currentStorageVersion: Future[StorageVersion] = {
    store.load(storageVersionName).map { variable =>
      Try(StorageVersion.parseFrom(variable.bytes)).getOrElse(StorageVersions.empty)
    }
  }

  def storeCurrentVersion: Future[StorageVersion] = {
    store.load(storageVersionName) flatMap { variable =>
      store.save(variable.mutate(StorageVersions.current.toByteArray)) map { newVar =>
        StorageVersion.parseFrom(newVar.bytes)
      }
    }
  }
  ...
  private def changeTasks(fn: InternalApp => InternalApp): Future[Any] = {
    ...
    def fetchApp(appId: PathId): Option[InternalApp] = {
      val bytes = Await.result(store.load("tasks:" + appId.safePath), config.zkTimeoutDuration).bytes
      if (bytes.length > 0) {
